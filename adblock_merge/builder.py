    ...
    for attempt in range(1, retries + 1):
        try:
            request = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "adblock-rule-merge/0.1 (+https://github.com/)",
                },
            )
...
def shadowrocket_module_header(part_index: int) -> str:
    return (
        f"#!name=Adblock Rule Merge Part {part_index}\n"
        "#!desc=Adblock reject rules, generated by adblock-rule-merge.\n"
        "[Rule]\n"
    )


def render_shadowrocket_module_text(rules: Iterable[ParsedRule], part_index: int) -> str:
    lines = [f"{rule.render()},REJECT" for rule in rules]
    return shadowrocket_module_header(part_index) + "\n".join(lines) + "\n"


def split_rendered_lines(
    lines: list[str],
    part_count: int,
    max_bytes: int,
    headers: list[str] | None = None,
) -> list[str]:
    headers = headers or ["" for _ in range(part_count)]
    if len(headers) != part_count:
        raise ValueError("headers length must match part_count")
    parts: list[list[str]] = [[] for _ in range(part_count)]
    part_sizes = [len(header.encode("utf-8")) for header in headers]
    current_part = 0
    ...
    return [header + "".join(part) for header, part in zip(headers, parts)]
...
def write_shadowrocket_parts(rules: list[ParsedRule], output_dir: Path) -> list[dict]:
    ...
    headers = [shadowrocket_module_header(index) for index in range(1, SHADOWROCKET_PART_COUNT + 1)]
    parts = split_rendered_lines(
        lines,
        part_count=SHADOWROCKET_PART_COUNT,
        max_bytes=SHADOWROCKET_MAX_BYTES,
        headers=headers,
    )
    part_reports: list[dict] = []

    for index, text in enumerate(parts, start=1):
        path = output_dir / f"reject-part{index}.module"
        path.write_text(text, encoding="utf-8", newline="\n")
        rule_count = max(0, len([line for line in text.splitlines() if line and not line.startswith("#!")]) - 1)
        part_reports.append(
            {
